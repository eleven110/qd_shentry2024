...
namespace navi_multi_goals_pub_rviz_plugin {
    ...
    MultiNaviGoalsPanel::MultiNaviGoalsPanel(QWidget *parent)
        ...
        nh_.param<std::string>("mutigoal_file_name",mutigoal_file_name,"mutigoalfile");
...
//        pose_array_.poses.resize(maxNumGoal_);
    ...
    void MultiNaviGoalsPanel::goalCntCB(const geometry_msgs::PoseStamped::ConstPtr &pose) {
        if (pose_array_.poses.size() < maxNumGoal_) {
            ...
            writePose(pose->pose);
            markPose(pose);
            px.push_back(pose->pose.position.x);
            py.push_back(pose->pose.position.y);
            pz.push_back(pose->pose.position.z);
            orientation_x.push_back(pose->pose.orientation.x);
            orientation_y.push_back(pose->pose.orientation.y);
            orientation_z.push_back(pose->pose.orientation.z);
            orientation_w.push_back(pose->pose.orientation.w);

            ROS_INFO("x = %f, y =%f, z=%f\n",pose->pose.position.x,pose->pose.position.y,pose->pose.position.z);
            ROS_INFO("orientation_x = %f, orientation_y =%f, orientation_z=%f\n",pose->pose.orientation.x,pose->pose.orientation.y,pose->pose.orientation.z);
            // std::string naviFileName = ros::package::getPath("navi_multi_goals_pub_rviz_plugin")+"/file/"+mutigoal_file_name+".csv";
            std::string naviFileName = ros::package::getPath("navi_multi_goals_pub_rviz_plugin")+"/file/"+mutigoal_file_name+".csv";
            std::ofstream myfile(naviFileName, std::ios::binary);
            for(int i =0; i<px.size();i++){
                myfile<<px[i]<<","<<py[i]<<","<<pz[i]<<","<<orientation_x[i]<<","<<orientation_y[i]<<","<<orientation_z[i]<<","<<orientation_w[i]<<std::endl;
            }      
            myfile.close();
        } else {
